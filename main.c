...
#define DEVICE_NAME                     "Nordic_neopixel"                               /**< Name of device. Will be included in the advertising data. */
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ...
    switch(p_data[0])
	{
		...
		case 'C':
			...
			break;
		case 'B':
			printf("Command: Brightness\n");
			ble_nus_string_send(&m_nus, (uint8_t *)"Ok", 2);
			break;
		case 'P':
			printf("Command: Pixel\n");
			...
			{
				printf("Color at [%d, %d]: (%d, %d, %d)\n", x, y, p_data[i], p_data[i+1], p_data[i+2]);
